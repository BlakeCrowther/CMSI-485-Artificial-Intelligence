...
class MazeProblem:
    ...
    def transitions(self, state):
        transition = []
        if self.maze[state[1] - 1][state[0]] != "X":
            transition.append(("U", (state[0], state[1] - 1)))
        if self.maze[state[1] + 1][state[0]] != "X":
            transition.append(("D", (state[0], state[1] + 1)))
        if self.maze[state[1]][state[0] - 1] != "X":
            transition.append(("L", (state[0] - 1, state[1])))
        if self.maze[state[1]][state[0] + 1] != "X":
            transition.append(("R", (state[0] + 1, state[1])))
        return transition
